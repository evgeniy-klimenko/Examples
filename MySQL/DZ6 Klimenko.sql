...
CREATE TABLE orders (id INTEGER, product_id INTEGER, product_count INTEGER);
...
p.price*o.product_count AS cost,
p.price AS average_price /*так как цена на определенные продукты устновлена одна и та же*/
FROM products p INNER JOIN orders o ON o.id = p.product_id
ORDER BY o.id ASC;

/* у меня не работает sakila, поэтому писал, как понял*/
SELECT CONCAT(c.first_name, c.last_name) AS name, cy.country
FROM customer c INNER JOIN country cy ON c.addres_id = cy.country_id
HAVING c.addres_id = MAX(addres_id);